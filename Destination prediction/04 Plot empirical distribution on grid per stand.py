...
if __name__ == "__main__":
  ...
  filepath_processed = "E:/MSc thesis/Processed data/grid_distribution_stands.csv"
  N, M = (100, 75)
  ...
  data_chunks = pd.read_csv(filepath_or_buffer = filepath,
                            sep = ",",
                            chunksize = 10000,
                            usecols = ["GRID_POLYLINE", "ORIGIN_STAND"],
                            converters = {'GRID_POLYLINE': lambda x: eval(x)})
                            
  for idx, chunk in enumerate(data_chunks):
    for trip_id, trip in chunk.iterrows():
      if trip.ORIGIN_STAND in stands:
        dest = trip.GRID_POLYLINE[-1]
        grid[(int(trip.ORIGIN_STAND), dest[0], dest[1])] += 1
      
    print "-- Processed chunk %d" % idx
  
  # Store the data from the grid into a Pandas DataFrame for two different stands
  grid_data = pd.DataFrame({"STAND": [entry[0] for entry in grid.keys()],
                            "CELL": [(entry[1], entry[2]) for entry in grid.keys()],
                            "COUNT": grid.values()})
  ...
  # Save the data
  grid_data.to_csv(filepath_processed, columns = ["STAND", "CELL_ID", "COUNT", "LON_MIN", "LON_MAX", "LAT_MIN", "LAT_MAX"], index = False, header = True)
